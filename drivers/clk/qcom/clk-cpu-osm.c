...
#define OC_PLL_L_VAL	0x04
#define OC_PLL_MODE	0x00
#define OC_UPDATE	BIT(22)
...
osm_cpufreq_target_index(struct cpufreq_policy *policy, unsigned int index)
{
...

	{
		...
		if (freq == 2496000)
			oc_lval = 130;
		else if (freq == 3091200)
			oc_lval = 161;

		if (oc_lval) {
			u32 mode;

			pr_emerg("OSM_OC: lval=%u clus=%d freq=%u\n",
				 oc_lval, c->cluster_num, freq);

			writel_relaxed(oc_lval, c->vbase + OC_PLL_L_VAL);
			mode = readl_relaxed(c->vbase + OC_PLL_MODE);
			writel_relaxed(mode | OC_UPDATE, c->vbase + OC_PLL_MODE);
			udelay(10);
			writel_relaxed(mode & ~OC_UPDATE, c->vbase + OC_PLL_MODE);
		}
	}
...
static int osm_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	for (i = 0; i < parent->osm_table_size; i++) {
		...
		if (table[i].frequency == CPUFREQ_ENTRY_INVALID)
			pr_emerg("CPUFREQ[CPU%d][%02d] freq=INVALID  flags=0x%x\n",
				 policy->cpu, i, table[i].flags);
		else
			pr_emerg("CPUFREQ[CPU%d][%02d] freq=%10u KHz  flags=0x%x\n",
				 policy->cpu, i, table[i].frequency,
				 table[i].flags);
	}

	/* Overclock: boost cpufreq table entries */
	if (parent->cluster_num == 2 || parent->cluster_num == 3) {
		int last = i - 1;
		if (last >= 0 && table[last].frequency == 2419200 &&
		    parent->cluster_num == 2) {
			table[last].frequency = 2496000;
			table[last].driver_data = 2496000;
		...
		if (last >= 0 && table[last].frequency == 2841600 &&
			...
			table[last].frequency = 3091200;
			table[last].driver_data = 3091200;
			pr_emerg("CPUFREQ_OC: CPU%d entry %d boosted to %u KHz\n",
				 policy->cpu, last, table[last].frequency);
		}
	}
...
static const char * const osm_cluster_names[] = {
	"L3", "pwrcl", "perfcl", "perfpcl",
};
...
static int clk_osm_read_lut(struct platform_device *pdev, struct clk_osm *c)
{
	...
	pr_emerg("=== OSM LUT: %s (cluster=%d) ===\n",
		osm_cluster_names[c->cluster_num], c->cluster_num);
	...
	for (i = 0; i < c->osm_table_size; i++) {
		...
		pr_emerg("OSM[%s][%02d] freq=%10lu Hz  lval=%3u  volt=%4umV  vcorner=%u\n",
			 osm_cluster_names[c->cluster_num],
			 i, c->osm_table[i].frequency, lval,
	...
	/* Overclock: boost highest entries */
	if (j > 0) {
		if (c->cluster_num == 2) {
			c->osm_table[j - 1].frequency = 2496000000UL;
			pr_emerg("OSM_OC: perfcl entry %zu boosted to %lu Hz\n",
				 j - 1, c->osm_table[j - 1].frequency);
		} else if (c->cluster_num == 3) {
			c->osm_table[j - 1].frequency = 3091200000UL;
			...
			if (j > 0) {
				...
				u32 vd2 = clk_osm_read_reg(c, voff);
				u32 ol2 = vd2 & GENMASK(11, 0);
				pr_emerg("OSM_OC: perfpcl volt %umV write %umV readback %umV\n",
					 ol, nv, ol2);
			}
		}
